<script lang="ts" type="module">
  ...
  let tierContract: string = "0x6ba1fadb694e806c316337143241dd6cfebd5033",
		multipleCheck = false,
    FriendlySource,
    deployPromise: any;
...
<div class="flex w-full gap-x-3">
  <div class="flex w-3/5 flex-col gap-y-4">
    <div class="mb-2 flex flex-col gap-y-2">
      <span class="text-2xl">Deploy a new "Has Tier Been Held For Blocks" from a TransferTier.</span>
      <span class="text-gray-400">
				Choose a TransferTier contract address to deploy a CombineTier that will produce a report out of if the tiers
				have been held for x number of blocks or not.
      </span>
    </div>
      ...
      >
		...
		<FormPanel>
			<div>
				...
				<br />
				<span class="text-gray-400"
					>if switched off only one value will be used for all 8 tiers to compare against TransferTier report.<br>
					if switched on each tier can have its own value to be compared agaoanst TransferTier report, however each provided value should be less than or equal to the previous one.</span
				>
			</div>
			<div id="single" style="display:none">
				<Input type="number" placeholder="Tier 1" bind:value={blocks[0]}>
					<span slot="label"
						>Set the time span in blocks for each to cpare against the TransferTier report.</span
					>
				</Input>
			</div>
			<div id="multiple" class="flex w-full flex-col space-y-3" style="display:none">
				<Input type="number" placeholder="Tier 1" bind:value={blocks[0]}>
					<span slot="label"
						>Set the time span in blocks for each tier which will be compared against the TransferTier report.</span
					>
				</Input>
